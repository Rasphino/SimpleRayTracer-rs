...
use crate::{hitable::HitRecord, ray::Ray, vec3::{random_in_unit_sphere, Vec3}};
...
impl Material {
    ...
    pub fn scatter(self, ray: &Ray, hit: &HitRecord) -> Scatter {
        ...
        match hit.material {
            Material::Lambertian(l) => l.scatter(ray, hit, &mut rng),
            Material::Metal(m) => m.scatter(ray, hit, &mut rng),
            Material::Dielectric(d) => d.scatter(ray, hit, &mut rng)
        }
    }
}